...
@section('title', 'Create Party Fuel Rate')

@section('content')
<div class="container mx-auto px-4">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Create Party Fuel Rate</h1>
        <a href="{{ route('s2e.party-fuel-rates.index') }}" class="text-gray-600 hover:text-gray-800">← Back to List</a>
    </div>

    <div class="bg-white rounded shadow p-6">
        <form action="{{ route('s2e.party-fuel-rates.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="fuel_rate" step="0.01" min="0" value="{{ old('fuel_rate') }}" 
                ...
                <div>
                    ...
                    <select name="rate_type" class="w-full border rounded px-3 py-2 @error('rate_type') border-red-500 @enderror" required>
                        <option value="per_kg" {{ old('rate_type') == 'per_kg' ? 'selected' : '' }}>Per KG</option>
                        <option value="per_cn" {{ old('rate_type') == 'per_cn' ? 'selected' : '' }}>Per CN</option>
